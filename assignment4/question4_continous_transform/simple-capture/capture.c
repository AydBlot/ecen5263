...
struct timespec process_times[30];
int process_times_count = 0;
static void process_image(const void *p, int size)
{
    ...
    process_times[process_times_count] = frame_time;

    if(process_times_count >= 1){
	    time_spent = (process_times[process_times_count].tv_sec - process_times[process_times_count-1].tv_sec) +
                         (process_times[process_times_count].tv_nsec - process_times[process_times_count-1].tv_nsec) / 1000000000.0; 
	    syslog(LOG_ERR,"frame rate is:%fs for frame:%d", time_spent, process_times_count-1);
    }
