...
static void dump_ppm(const void *p, int size, unsigned int tag, struct timespec *time)
{
    ...
    char ppm_sharpen_dumpname[50]={0};
...
#if defined(SHARPEN)
    sprintf(ppm_sharpen_dumpname, "sharpen_%s", ppm_dumpname);
    printf("sharpen name:%s\r\n", ppm_sharpen_dumpname);
    sharpen(ppm_dumpname, ppm_sharpen_dumpname);
#endif    
}
...
#if defined(SHARPEN)
    char pgm_sharpen_dumpname[50]={0};
    sprintf(pgm_sharpen_dumpname, "sharpen_%s", pgm_dumpname);
    printf("sharpen name:%s\r\n", pgm_sharpen_dumpname);
    sharpen(pgm_dumpname, pgm_sharpen_dumpname);
#endif    
}
...
static void process_image(const void *p, int size)
{
    ...
    static double frame_rates[29] = {0};
    char time_buf[50];
    ...
    if(process_times_count >= 1){
	    ...
	    syslog(LOG_ERR,"frame rate is:%fs for frame:%d at resolution 320x240 for transform type greyscale", frame_rates[process_times_count], process_times_count-1);
    }
...
static void uninit_device(void)
{
        ...
        case IO_METHOD_MMAP:
			...
			buffers[i].start = 0;
			buffers[i].length = 0;
			printf("uninit case IO_METHOD_MMAP\r\n");
			printf("TEST\r\n");
			printf("n_buffers:%d\r\n", n_buffers);
	...
	printf("uninit device before free\r\n");
        free(buffers);
	printf("uninit device after free\r\n");
}
...
static void init_device(void)
{
	...
	//fmt.fmt.pix.pixelformat = 4L2_PIX_FMT_GREY;
    ...
    switch (io)
    {
        ...
        case IO_METHOD_MMAP:
            init_mmap();
	    printf("Initialized MMAP for buffer\r\n");
            break;
...
int main(int argc, char **argv)
{
    ...
    openlog("capture.c", LOG_PID|LOG_CONS, LOG_USER);
	...
	//Find the highest priority of the FIFO scheduler
	rt_max_prio = sched_get_priority_max(SCHED_FIFO);
	rt_min_prio = sched_get_priority_min(SCHED_FIFO);
	printf("rt_max_prio=%d\n", rt_max_prio);
	printf("rt_min_prio=%d\n", rt_min_prio);

	//set the scheduler type to be FIFO
	...
	//Double check the scheduler was set properly
	int schedType = sched_getscheduler(getpid());
	if(schedType == SCHED_FIFO){
		printf("Scheduler type is SCHED_FIFO\r\n");
	} 
	else{
		printf("Scheduler assignment incorrect\r\n");
	}

	//select CPU core 3 for the threads.
	CPU_ZERO(&threadcpu);
	CPU_SET(3, &threadcpu);

	//Set the scheduler policy and cpu for each thread
	...
	rc=pthread_attr_setaffinity_np(&rt_sched_attr, sizeof(cpu_set_t), &threadcpu);

	//Set the scheduling priority for a lower thread each time 
	rt_param.sched_priority=rt_max_prio;
	pthread_attr_setschedparam(&rt_sched_attr, &rt_param);


	if(pthread_create(&thread, &rt_sched_attr, mainloop, NULL) < 0) {
